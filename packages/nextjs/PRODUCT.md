...
- **Cada pantalla tiene URL propia y la raíz no es una pantalla: reparte.** `/login` es la única puerta —de los dos lados—, `/rol` es la única que pregunta el lado, `/oportunidades` es el catálogo y `/solicitar` el panel de empresa. `/` mira la sesión y manda a la que corresponde, que es lo que hace que el QR del stand pueda apuntar al dominio pelado y funcione para quien llega por primera vez y para quien vuelve.
- El primer contacto no tiene chrome ni formulario: un botón crea la wallet con el correo y entra. El lado se elige después, en `/rol`, con la wallet ya conectada — preguntarlo antes obligaba a transportar la respuesta por `localStorage`, hacerla sobrevivir al modal de Privy, caducarla y reconciliar el choque cuando la cuenta pertenecía al otro lado. Al inversionista se le acredita saldo de prueba solo, en una franja que no detiene el catálogo; al dueño de negocio no. Lo ya explicado no se repite (onboarding marcado en `localStorage`).
- **Una wallet pertenece a un solo lado y eso no cambia.** Cuando alguien pide entrar por el otro, se le dice y se le ofrecen las dos salidas reales (seguir en el suyo, o entrar con otra cuenta); no se lo redirige en silencio.
